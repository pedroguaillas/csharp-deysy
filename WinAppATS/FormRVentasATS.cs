...
namespace WinAppATS
{
    public partial class FormRVentasATS : Form
    {
        public FormRVentasATS()
        {
            InitializeComponent();
        }

        private void FormRVentasATS_Load(object sender, EventArgs e)
        {
        ...
        private async void reporte()
        {
            ...
            DataSet data = await reportDate.ventas();

            if (data.Tables.Count > 0)
            {
                DataRow[] rowsSelect = data.Tables[0].Select("ruc <> ''");
                ...
                {
                    MessageBox.Show("Al momento solo tiene ventas anuladas o no se ha registrado la identificación del cliente", "Reporte de ventas", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
            }
            else
            {
                MessageBox.Show("No se ha registrado o guardo las ventas", "Reporte de ventas", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }
    }
}